import {
  ...
  export const createSubject = async (req, res, next) => {
    const { course_id, subject_title } = req.body;
    ...
    try {
      const newSubject = await createSubjectService(course_id, subject_title);
  ...
  export const updateSubject = async (req, res, next) => {
    ...
    try {
      const updatedSubject = await updateSubjectService(id, subject_title);
      if (!updatedSubject) {
        return handleResponse(res, 404, "Subject not found");
      }
